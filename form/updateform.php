...
<div class="form m-3  ">
<form action="display/update.php" id="myform" method="post" class="mt-5 " accept-charset="utf-8"  enctype="multipart/form-data">
...
if (isset($_GET['error'])) {
 ...
 <input type="hidden" name="id" value="'.$_SESSION['id'].'">
  ...
  <div class="mb-3">
  <label > the image must be less that <strong>4 MB</strong> for the main image</label>
  <input type="file" class="form-control" id="image"  name="mainimage" placeholder="'.$_SESSION["mainimage"].'">
  </div>
...
<h3 class="text-white text-capitalize text-center"><?php 
...
function saveFormData() {
  const formData = document.getElementById('myForm').elements;
  for (let i = 0; i < formData.length; i++) {
    localStorage.setItem(formData[i].name, formData[i].value);
  }
}
...
function retrieveFormData() {
  const formData = document.getElementById('myForm').elements;
  for (let i = 0; i < formData.length; i++) {
    formData[i].value = localStorage.getItem(formData[i].name);
  }
}

// Call saveFormData() when the form is submitted
document.getElementById('myForm').addEventListener('submit', saveFormData);

// Call retrieveFormData() when the page is loaded
window.addEventListener('load', retrieveFormData);
